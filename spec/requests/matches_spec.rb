...
RSpec.describe 'Matches' do
  describe 'non registered user management' do
    ...
    it 'can GET gallery' do
      get gallery_path
      expect(response).to be_successful
    end

    it 'cannot GET gallery_show without season_id and redirect to gallery' do
      get gallery_show_path
      expect(response).to redirect_to(gallery_path)
    end

    it 'can GET gallery_show with params[:season_id]' do
      season = create(:season)
      get gallery_show_path(season_id: season.id)
      expect(response).to be_successful
    end
  end
  ...
  describe 'user-admin management' do
    ...
    it 'can POST create' do
      ...
      team = create(:team)
      post matches_path, params: { match: attributes_for(:match, tournament_id: tournament.id, season_id: season.id,
                                                                 stadium_id: stadium.id, home_team_id: team.id,
